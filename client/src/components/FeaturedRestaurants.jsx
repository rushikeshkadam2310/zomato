...
const FeaturedRestaurants = () => {
  ...
  useEffect(() => {
    const fetchFeaturedRestaurants = async () => {
      try {
        const response = await axios.get('http://localhost:5000/restaurants/featured');
    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      <div className="rounded-2xl p-10 w-full max-w-screen-xl">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 text-lg">Discover top-rated dining spots handpicked for you</p>
        </div>
        <nav className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {featuredRestaurants.map((restaurant, index) => (
            ...
            >
              ...
              <p className="text-gray-600 mt-2 text-center">{restaurant.locality}</p>
            </Link>
          ))}
        </nav>
      </div>
