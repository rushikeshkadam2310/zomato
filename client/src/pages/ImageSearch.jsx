    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 p-4 md:p-8">
      <div className="max-w-full md:max-w-4xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        <div className="bg-gradient-to-r from-blue-500 to-purple-600 p-6 md:p-8 text-white">
          <h2 className="text-2xl md:text-3xl font-bold mb-4 text-center">Search Restaurants by Image</h2>
        </div>
        ...
        <div className="p-4 md:p-8">
          <div className="flex items-center justify-center mb-6">
            <label htmlFor="imageUpload" className="cursor-pointer flex flex-col items-center">
              ...
              <span className="text-lg text-gray-600 hover:text-gray-800">Upload an image</span>
              <input
                id="imageUpload"
                type="file"
          ...
          {imageName && <p className="text-center text-gray-700 mb-4">Selected Image: {imageName}</p>}
          <button
          ...
          {isLoading ? (
            ...
            <div className="mt-8">
              {searchMade && restaurants.length === 0 ? (
                <p className="text-gray-500 text-center py-12">No restaurants found for the selected image.</p>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
                  {restaurants.map((restaurant) => (
                    <Link key={restaurant.restaurantId} to={`/restaurants/${restaurant.restaurantId}`} className="block group">
                      <div className={`bg-gradient-to-br ${getRandomColor()} p-4 md:p-6 rounded-lg shadow-lg transform transition-transform duration-300 hover:scale-105`}>
                        <MdRestaurant className="text-3xl md:text-4xl text-white mb-4" />
                        <h3 className="text-lg md:text-xl font-semibold text-white mb-2">{restaurant.restaurantName}</h3>
                        <p className="text-white opacity-80">{restaurant.cuisines}</p>
                      </div>
                    </Link>
                  ))}
                </div>
