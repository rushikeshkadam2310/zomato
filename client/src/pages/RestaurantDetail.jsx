...
import { MdLocationOn, MdPlace, MdFastfood, MdAttachMoney, MdMoneyOff, MdStar, MdThumbUp } from 'react-icons/md';
...
const fetchRestaurant = async () => {
  try {
    const response = await axios.get(`https://zomato-kgmy.onrender.com/restaurants/${id}`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-center">
          ...
          <p className="mt-4 text-xl text-red-800">Loading delicious details...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-3xl shadow-2xl overflow-hidden">
        <div className="relative h-96">
          ...
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-end">
            <div className="p-8">
              <h2 className="text-5xl font-bold text-white mb-2">{restaurant.restaurantName}</h2>
        ...
        <div className="p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: MdLocationOn, title: "Address", value: `${restaurant.address}, ${restaurant.city}`, color: "bg-blue-100 text-blue-800" },
              { icon: MdPlace, title: "Locality", value: restaurant.locality, color: "bg-green-100 text-green-800" },
              { icon: MdFastfood, title: "Cuisines", value: restaurant.cuisines, color: "bg-yellow-100 text-yellow-800" },
              { icon: MdAttachMoney, title: "Average Cost for Two", value: `${restaurant.currency} ${restaurant.averageCostForTwo}`, color: "bg-purple-100 text-purple-800" },
              { icon: MdMoneyOff, title: "Price Range", value: restaurant.priceRange, color: "bg-red-100 text-red-800" },
              { icon: MdStar, title: "Rating", value: `${restaurant.aggregateRating} (${restaurant.ratingText})`, color: "bg-teal-100 text-teal-800" },
              { icon: MdThumbUp, title: "Votes", value: restaurant.votes, color: "bg-indigo-100 text-indigo-800" },
            ].map((item, index) => (
              <div key={index} className={`${item.color} rounded-xl p-6 shadow-lg transform hover:scale-105 transition-all duration-300`}>
          ...
          <div className="mt-12">
            <h3 className="text-3xl font-semibold text-gray-800 mb-6">Location</h3>
