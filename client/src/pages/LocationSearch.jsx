...
function LocationSearch() {
  ...
  const handleSearch = async () => {
    ...
    if (latitude && longitude && radius) {
      ...
      setInputError('');
      try {
        const response = await axios.get('http://localhost:5000/restaurants/search/location', {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 p-4 md:p-8">
      <div className="max-w-full md:max-w-4xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        <div className="bg-gradient-to-r from-blue-500 to-purple-600 p-6 md:p-8 text-white">
          ...
          <p className="text-base md:text-lg opacity-90">Discover delicious dining options in your area</p>
        </div>
              ...
              {loading ? (
                ...
                <div className="mt-6">
                  {restaurants.length === 0 ? (
                    <p className="text-gray-500 text-center py-12">No restaurants found in your locality.</p>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
                      {restaurants.map((restaurant) => (
                        <Link key={restaurant.restaurantId} to={`/restaurants/${restaurant.restaurantId}`}
                          ...
                          <div className={`bg-gradient-to-br ${getRandomColor()} p-4 md:p-6 rounded-lg shadow-lg transform transition-transform duration-300 hover:scale-105`}>
                            ...
                            <p className="text-white opacity-80">{restaurant.locality}</p>
                          </div>
                        </Link>
                      ))}
                    </div>
